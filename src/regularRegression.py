...
if __name__ == "__main__":
    ...
    # regParam * (elasticNetParam*weight_1_norm + (1-elasticNetParam)*weight_2_norm))
    print ("#--------- regularized linear regression -------------------------------------")
    lr_r = LinearRegression(maxIter=10, regParam=0.3, elasticNetParam=0.8)
    
    pipeline_r = Pipeline(stages=[featureIndexer, lr_r])
    model_r = pipeline_r.fit(trainingData)
    
    predictions_r = model_r.transform(testData)
    predictions_r.select("features", "label", "prediction").show(5)

    # Select (prediction, true label) and compute test error
    rmse = evaluator.evaluate(predictions_r)
    print("Root Mean Squared Error (RMSE) on test data = %g" % rmse)
